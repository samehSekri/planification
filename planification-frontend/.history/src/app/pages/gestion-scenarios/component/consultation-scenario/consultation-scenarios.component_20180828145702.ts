...
public getAllScenarios(): void {
 ...
 /* this.scenarioService.getByUnite( this.currentUnite).subscribe((data: Scenario[]) => {
  ...
  error => console.log(error),
  () => console.log('Get all Items complete'));*/
 this.scenario=this.getScenarioByUnite()
  for(this.scenario in this.scenarios){
    if(this.scenario.statut==this.statutScenario){
  this.scenariosUnite.push(this.scenario)
    }
    }}else{
 this.getScenarioByUnite();}

...
export class ConsulationScenarioComponent implements OnInit {
  ...
  public getAllScenarios(): void {
    if(this.currentUnite==null){
      this.unite=this.currentAtelier;
    }
    let filterScenario = new FilterScenario(this.dateValue, this.statutScenario, this.unite);
    this.scenarioService.getByCriteria(filterScenario).subscribe((data: Scenario[]) => {
      this.scenarios = data;

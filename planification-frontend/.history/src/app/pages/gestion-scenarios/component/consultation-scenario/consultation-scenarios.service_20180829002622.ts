...
import { FilterScenario } from 'app/shared/model/filterScenario.model';
...
export class ConsultationScenarioService {
 ...
 /**
  ...
  public getByCriteria = (filter: FilterScenario): Observable<Scenario[]> => {
      
    return this.http.post(this.actionUrl  +"/scenarioStatut",filter ,this.options())
      .map((response: Response) => <Scenario[]>response.json())
      .catch((response: Response) => this.errorHandler(response));
  }

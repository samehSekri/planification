...
public uploadExcel = (file: FormData): Observable<Response> => {
  this.headers = new Headers({ 'Authorization': this.authenticationService.getToken(), 'content': 'application/json'});
  let options = new RequestOptions({ headers: this.headers});
 // return this.http.post(this.actionUrl + '/upload-excel', file, options)
  return this.http.post(this.actionUrl + '/upload-excel', file , options).map(res => res.json().data);
 // return this.http.post(this.actionUrl + '/upload-excel', file, options)
